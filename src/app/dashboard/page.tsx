...
export default function DashboardPage() {
  ...
  const radarData = quizData?.map((q: any) => {
    const labels: Record<string, string> = { bigfive: "人格", lovelanguage: "爱的语言", travel: "旅伴", marriage: "婚姻觀" }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-2">匹配分析仪表板</h1>
      <p className="text-gray-500 mb-8">了解你的匹配数据和心理学测验分析</p>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { icon: Users, label: "推荐人数", value: matchStats.total, color: "#FF6B8A" },
          { icon: TrendingUp, label: "平均匹配", value: `${matchStats.avg}%`, color: "#6366f1" },
          { icon: Star, label: "最高匹配", value: `${matchStats.top}%`, color: "#f59e0b" },
          { icon: Heart, label: "完成测验", value: `${quizData?.length || 0}/4`, color: "#10b981" },
        ].map((s, i) => (
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <h3 className="font-bold mb-4">心理学测验概览</h3>
          {radarData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <RadarChart data={radarData}>
                ...
                <Radar name="你的分数" dataKey="value" stroke="#FF6B8A" fill="#FF6B8A" fillOpacity={0.3} />
              </RadarChart>
            </ResponsiveContainer>
          ) : (
            <div className="h-[300px] flex items-center justify-center text-gray-400">完成测验后显示数据</div>
          )}
        </div>

        {/* Match Distribution */}
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <h3 className="font-bold mb-4">匹配度分布</h3>
          <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="mt-8 p-6 rounded-2xl bg-pink-50">
        <h3 className="font-bold mb-3" style={{ color: "#FF6B8A" }}>💡 AI 相处建议</h3>
        <ul className="space-y-2 text-sm text-gray-600">
          <li>• 完成更多心理学测验可以提升匹配精准度</li>
          <li>• 72 小时内见面是关系建立的最佳时机</li>
          <li>• 在聊天中展现真实的自己，建立信任感</li>
          <li>• 注意彼此的爱的语言差异，用对方喜欢的方式表达关心</li>
        </ul>
      </div>
    </div>
  )
