...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (form.password !== form.confirmPassword) { setError("密碼不一致"); return }
    if (form.password.length < 6) { setError("密碼至少 6 位"); return }
    setLoading(true)
    setError("")
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2">用心理學，找到對的人</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="你的暱稱" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
            ...
            <div className="relative">
              ...
              <input type="password" placeholder="密碼（至少 6 位）" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })}
                className="w-full pl-11 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-200 transition" required />
            </div>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
              <input type="password" placeholder="確認密碼" value={form.confirmPassword} onChange={e => setForm({ ...form, confirmPassword: e.target.value })}
                className="w-full pl-11 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-200 transition" required />
            </div>
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            已有帳號？<Link href="/login" className="font-medium" style={{ color: "#FF6B8A" }}>登入</Link>
          </p>
        </div>
      </div>
    </div>
